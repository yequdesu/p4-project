...
def main():
    ...
    parser.add_argument('--all', action='store_true', help='Run all receivers (IPv4, IPv6, source routing)')

    args = parser.parse_args()

    if args.all:
        # Run all receivers in parallel
        print("Starting all receivers (IPv4, IPv6, source routing)...")
    ...
    elif args.both or (not args.ipv4 and not args.ipv6 and not args.src_route):
